...
- [ ] Move non-boundary imports to the top of the file: `import yaml` in `config.py:load_config`, `import io` mid-file in `test/test_tracer_node.py`. (The lazy ROS imports in `msg_type.py`/`qos_select.py` are deliberate boundary isolation — keep those.)
