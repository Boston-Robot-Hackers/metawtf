...
from dome_nav.frontier_params import FrontierTuning, merge_tuning
...
class ExplorerManagerNode(Node):
    ...
    def session_start_params(self) -> dict:
        # Session telemetry. The shared/session params are always logged; frontier
        # tuning is added only when the active algorithm carries it (moves off the
        # node entirely in F23 T02).
        params: dict = {
        ...
        tuning = self.frontier_tuning()
        if tuning is not None:
            params.update({
                "min_frontier_dist": tuning.min_frontier_dist,
                "max_frontier_dist": tuning.max_frontier_dist,
                "goal_inset": tuning.goal_inset_m,
                "min_frontier_size": tuning.min_frontier_size,
            })
        return params

    def frontier_tuning(self) -> FrontierTuning | None:
        # Combined frontier tuning for the still-frontier-coupled diagnostics /
        # marker / telemetry paths (those move off the algorithm in F23 T02). Built
        # from the shared params plus the algorithm's own frontier params; None for
        # algorithms that carry no frontier tuning.
        frontier_params = getattr(self.algorithm, "frontier_params", None)
        if frontier_params is None:
            return None
        return merge_tuning(self.params, frontier_params)
    ...
    def dump_frontier_exhaustion(self, robot_xy: XY):
        ...
        self.get_logger().info(format_frontier_exhaustion(
            ...
            tuning, self.blacklist, self.NO_FRONTIER_PATIENCE,
        ))
    ...
    def publish_markers(self):
        # min_frontier_size only filters frontier clusters; a non-frontier
        # algorithm has none, so any value is inert there (marker ownership moves
        # to the algorithm in F23 T02).
        tuning = self.frontier_tuning()
        min_frontier_size = tuning.min_frontier_size if tuning is not None else 0
